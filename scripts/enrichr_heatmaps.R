...
library(tidyverse) # includes ggplot2, for data visualisation. dplyr, for data manipulation, stringr, for string manipulation

# Write the log to the snakemake log file.
# Adapted from:
# https://stackoverflow.com/a/64114743/4277966

log <- file(snakemake@log[[1]], open = "wt")
sink(log)

# Import the data
enrichr_table <- read.delim(snakemake@input[[1]], header = TRUE, sep = "\t")
enrichr_term <- as.character(snakemake@params[[2]])
log2_mat <- as.matrix(read.table(snakemake@input[[2]], header = TRUE, row.names = 1, check.names = FALSE)) # diffExpr.P${fdr}_C${foldchange}.matrix
diff_genes <- read.delim(snakemake@input[[3]], header = TRUE, sep = "\t")
log2_comp <- read.delim(snakemake@input[[4]], header = FALSE, sep = "\t", row.names = 1) # sorted_logfolds.txt
out_dir <- as.character(snakemake@params[[1]])

# Save the enrichr_term to a new variable for uise in the file name
filename_term <- enrichr_term

# Replace underscores with spaces and make the first letter of the term uppercase
enrichr_term <- str_to_sentence(gsub("_", " ", enrichr_term))
...
# Change the term to lowercase and replace spaces with underscores
enrichr_term <- tolower(gsub(" ", "_", enrichr_term))

# Draw the combined heatmap in one line
png(paste(out_dir, "/enrichr_heatmap_", filename_term, ".png", sep = ""), height = 2000, width = 2000, res = 300); draw(heatmap_hclust_rep_rt + heatmap_hclust_rep_16); dev.off()
