...
# Keep only the following genes CTRL, MEP1B, CTRB1
prot_dig_and_abs <- diffexpressed_genes |>
  filter(top_blastp_hit %in% c("CTRL", "MEP1B", "CTRB1"))

# Save the protein digestion and absorption genes to a file
write.table(prot_dig_and_abs, "data/processed/DESeq2/DESeq2_gene/protein_digestion_and_absorption_genes.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
# ggsave(snakemake@output[[5]], kegg_plot, width = 2500, height = 2500, units = "px", dpi = 300)